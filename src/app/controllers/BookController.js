...
class BookController {
	...
	async createBook(req, res) {
		...
		if (error) {
			return res.json({ result: false, msg: error.details[0].message })
		}
	...
	async updateBook(req, res) {
		...
		const data = req.body
		const isChangingPhoto = req.body?.is_changing_photo
		delete data?.is_changing_photo

		const photos = []
		// Check whether or not User is changing photos of book
		if (isChangingPhoto !== 'false') {
			...
			await Promise.all(
				Array.from(files).map(async (file) => {
					...
					const storageRef = ref(
						storage,
						`photos/${curDate.getUTCMilliseconds()}${file.originalname}`
					)
