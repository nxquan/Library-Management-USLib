...
class RecordController {
	...
	async createRecord(req, res) {
		...
		if (readerCard) {
			...
			const left = Book.unicodeToASCII(readerCard.fullName)
			const right = Book.unicodeToASCII(data.name)

			if (left === right) {
				// Check if the expiration of account
				if (RecordController.isValidDate(readerCard.dateCreated, data.date, 6)) {
